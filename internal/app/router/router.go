...
import (
	...
	"github.com/ozonmp/omp-bot/internal/app/commands/security"
...
func NewRouter(
	...
	return &Router{
		...
		// rating
		// security
		securityCommander: security.NewSecurityCommander(bot),
		// cinema
		// logistic
		// product
		// education
...
func (c *Router) handleCallback(callback *tgbotapi.CallbackQuery) {
	...
	case "work":
		break
	case "service":
		c.securityCommander.HandleCallback(callback, callbackPath)
	case "exchange":
		break
	case "estate":
		break
...
func (c *Router) handleMessage(msg *tgbotapi.Message) {
	...
	case "rating":
		break
	case "security":
		c.securityCommander.HandleCommand(msg, commandPath)
	case "cinema":
		break
	case "logistic":
		break
