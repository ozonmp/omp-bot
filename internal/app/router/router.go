...
import (
	...
	"github.com/ozonmp/omp-bot/internal/app/commands/license"
...
func NewRouter(
	...
	return &Router{
		...
		bot: bot,
		// demoCommander
		//demoCommander: demo.NewLicenseCommander(bot),
		// user
		// access
		// buy
		// delivery
		...
		// bank
		// subscription
		license: license.NewLicenseCommander(bot),
		// insurance
		// payment
		// storage
		// streaming
...
func (c *Router) handleCallback(callback *tgbotapi.CallbackQuery) {
	...
	case "subscription":
		break
	case "license":
		c.license.HandleCallback(callback, callbackPath)
	case "insurance":
		break
	case "payment":
		break
...
func (c *Router) handleMessage(msg *tgbotapi.Message) {
	...
	case "subscription":
		break
	case "license":
		c.license.HandleCommand(msg, commandPath)
	case "insurance":
		break
	case "payment":
		break
...
func (c *Router) showCommandFormat(inputMessage *tgbotapi.Message) {
	outputMsg := tgbotapi.NewMessage(inputMessage.Chat.ID, "Command format: /{command}__{domain}__{author}")
